<script lang="ts">
    ...
    import { fetchCommentsByProfileAPI } from "$lib/ergo/commentStore";
    ...
    export let web_explorer_uri_tx: string =
    ...
    // Group comments by discussion (topic)
    ...
    $: sortedTopics = Object.keys(groupedComments).sort((a, b) => {
        // Sort by most recent comment in the topic
        const lastCommentA = groupedComments[a][0]; // Comments are already sorted by time desc
        const lastCommentB = groupedComments[b][0];
        return lastCommentB.timestamp - lastCommentA.timestamp;
    });

    function getAvatarSvg(tokenId: string, size = 64): string {
