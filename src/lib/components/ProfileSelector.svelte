<script lang="ts">
    ...
    import { reputation_proof, user_profiles } from "$lib/ergo/store";
    import type { ReputationProof } from "$lib/ergo/object";

    function selectProfile(profile: ReputationProof) {
    ...
    function profileTypeLabel(profile: ReputationProof): string {
        if (!profile.types || profile.types.length === 0) return "Proof";
    ...
    function ergBurned(profile: ReputationProof): string {
        // Reputation is "burned" ERG locked in the proof boxes.
        const total = profile.current_boxes.reduce((sum, box) => {
            const v = (box as any).value;
